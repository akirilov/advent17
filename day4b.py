...
puzzle_in = '''pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
puzzle_in_lines = puzzle_in.split('\n')

count = 0

for line in puzzle_in_lines:
    # Split into words
    line_words = line.split()
    # Split words into lists of letters
    line_ctrs = map(list, line_words)
    # Resort alphabetically
    map(lambda x: x.sort(), line_ctrs)
    line_ctrs_s = map(lambda x: "".join(x), line_ctrs)
    # Do the set trick again
    lcs_set = set(line_ctrs_s)
    if len(line_ctrs_s) == len(lcs_set):
        count += 1
